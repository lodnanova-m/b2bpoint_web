...
export function Header() {
    ...
    const handleHashLinkClick = (hash: string) => (e: MouseEvent<HTMLAnchorElement>) => {
        if (pathname !== '/') {
            e.preventDefault();
            router.push(`/${hash}`);
        }
    ...
    return (
        <div className=''>
            <div className="flex flex-row justify-between p-6 md:p-10 md:px-16 content-center pl-10">
                <Link href="/"><Image src="/logo_main.svg" alt="logo" width={110} height={40} /></Link>

                <div className="md:hidden flex items-center">
                    <button onClick={toggleMenu} className="focus:outline-none">
                        <Image src="/menu-burger.svg" alt='burger menu button' height={50} width={50}></Image>
                    </button>
                </div>
            ...
            <NavBarLinks handleHashLinkClick={handleHashLinkClick} menuOpen={menuOpen} closeMenu={closeMenu} />
        </div>
    );
}
